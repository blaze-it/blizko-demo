...
test.describe('Browse Events', () => {
	...
	test('shows empty state when no events match', async ({ page }) => {
		...
		await page.getByPlaceholder('Search events...').fill('zzzznonexistent999')
	...
	test('search filters events by title', async ({ page }) => {
		...
		await page.getByPlaceholder('Search events...').fill(uniqueTitle)
		await page.waitForTimeout(1000)
	...
	test('floating create button navigates to new event page', async ({ page }) => {
		await page.goto('/events')
		// The floating + button
